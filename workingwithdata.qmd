...
For this guide, we'll work with data that comes from administrative data sources, and already meets the tidy data standards, as it was collected and processed in machine readable formats.  That doesn't mean it won't need some cleaning, of course.
...
Lastly, since we want to use the `SECTOR` variable in particular later, let's provide it with descriptions that are used in IPEDS:^[And included in the dictionary file or the web interface]
